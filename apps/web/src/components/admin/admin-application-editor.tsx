...
export function AdminApplicationView({ id }: { id: string }) {
  ...
  return <main className="admin-shell admin-detail-page">
    ...
    {record && <section className="admin-card admin-record-card">
      ...
      <div className="admin-record-meta"><span>Session code <strong>{metadata?.sessionCode ?? "Not available"}</strong></span><span>Created {new Date(metadata?.createdAt ?? "").toLocaleString()}</span><span>Updated {new Date(metadata?.updatedAt ?? "").toLocaleString()}</span></div>
...
export function AdminApplicationEditor({ id }: { id: string }) {
  ...
  return <main className="admin-shell admin-detail-page"><AdminHeader title="Edit application" description="Make corrections directly to the saved record. Changes are applied to the database when you save." status="Admin edit mode" /><section className="admin-card admin-editor-card"><div className="admin-editor-intro"><div><p className="form-kicker">{draft.applicant.fullName || "Unnamed applicant"}</p><h2>Application information</h2><p className="admin-editor-help">Session code: <strong>{detail.data?.sessionCode ?? "Not available"}</strong></p></div>{saveState && <span className={saveState.startsWith("Could") ? "admin-save-error" : "admin-save-state"}>{saveState}</span>}</div>
