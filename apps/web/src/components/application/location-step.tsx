...
export function LocationStep({ value, defaultValue, onChange, onAvailabilityChange, readOnly = false }: { value: LocationValue; defaultValue: LocationValue; onChange: (value: LocationValue, defaultValue?: LocationValue) => void; onAvailabilityChange?: (canProceed: boolean) => void; readOnly?: boolean }) {
  ...
  const reverseGeocode = async (latitude: number, longitude: number, source: LocationValue["source"], isDefault = false) => {
    ...
    try {
      ...
      setQuery(address);
      onChange({ ...value, latitude, longitude, source, label: result.name || "Selected location", address }, isDefault ? { ...selected, label: result.name || "Selected location", address } : undefined);
  ...
  useEffect(() => { if (readOnly) { onAvailabilityChange?.(true); return; } if (!navigator.geolocation) { onAvailabilityChange?.(true); setStatus("This browser does not support geolocation. Choose a point on the map."); } else if (defaultValue.latitude === null && value.latitude === null) useDeviceLocation(true); else onAvailabilityChange?.(true); }, []);
  useEffect(() => { if (permissionDenied) setLocationPromptOpen(true); }, [permissionDenied]);

  return <div className="location-layout"><div className="form-panel"><div className="field-group"><label htmlFor="location-search">Where does the applicant live?</label><p className="field-help">Choose the applicant&apos;s home location. This is used to help determine the nearest school.</p><input id="location-search" value={query} onChange={(event) => { setQuery(event.target.value); onChange({ ...value, address: event.target.value, source: "manual" }); }} placeholder="Enter an address or landmark" /></div>{!readOnly && <button type="button" className="secondary-button full-button" onClick={() => useDeviceLocation(false)}><LocateFixed size={17} /> Use my current device location</button>}{status && <p className={permissionDenied ? "error-line" : "status-line"} role="status">{status}</p>}{value.latitude === null && !permissionDenied && <p className="field-help"><TriangleAlert size={16} /> Select a point on the map or use your device location to continue.</p>}</div><div className="map-frame" aria-label="OpenStreetMap location picker"><MapContainer center={point ?? DEFAULT_CENTER} zoom={point ? 13 : 7} scrollWheelZoom className="application-map"><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />{!readOnly && <MapSync point={point} onSelect={(lat, lng) => { if (!permissionDenied) void reverseGeocode(lat, lng, "map"); }} />}</MapContainer><div className="map-caption">Click the map to place the location pin</div></div><Dialog open={locationPromptOpen} onOpenChange={setLocationPromptOpen}><DialogContent><DialogHeader><DialogTitle>Allow location access</DialogTitle><DialogDescription>Browser location permission is required to continue. Enable location access for this site in your browser settings, then try again.</DialogDescription></DialogHeader><button type="button" className="primary-button" onClick={() => { setLocationPromptOpen(false); useDeviceLocation(true); }}><LocateFixed size={17} /> Try again</button></DialogContent></Dialog></div>;
}
