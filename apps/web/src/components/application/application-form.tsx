...
export function ApplicationForm({ adminApplicationId, readOnly = false }: { adminApplicationId?: string; readOnly?: boolean }) {
  ...
  return <main className="application-shell">
    <section className="application-intro"><div><p className="form-kicker">G1 2026 intake</p><h1>Applicant information</h1><p>Complete the details at your own pace. Your progress is saved securely and can be reopened with your session code and access key.</p>{sessionCode && <p className="application-key"><strong>Session code:</strong> <code>{sessionCode}</code><br /><span className="session-code-note">Memorise this code to find this child&apos;s application on another device.</span></p>}{accessKey && <p className="application-key"><strong>Access key:</strong> <code>{accessKey}</code><br /><span>This key authorizes you to view, change, and edit this application again. Store it safely.</span></p>}</div><div className="draft-badge"><ShieldCheck size={17} /> {accessKey ? "Saved to database" : "Connecting to database"}</div></section>
