...
import { useAdminPreferences } from "@/lib/g1/admin-preferences";
...
export const Route = createFileRoute("/_auth/g1/admin/applications")({
  ...
  return useMemo(() => [
    {
      ...
      cell: ({ row }: { row: { original: ApplicationRow } }) => (
        <Link to="/g1/admin/applications/$id" params={{ id: row.original.id }} className="font-semibold text-primary hover:underline">
          ...
          <CardContent className="py-3">
            ...
            <div className="flex flex-wrap gap-2">
              {prefs.recentlyViewed.map((entry) => (
                <Link key={entry.id} to="/g1/admin/applications/$id" params={{ id: entry.id }} className="inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs hover:bg-accent transition-colors">
