...
function AccessPage() {
  ...
  const upsertBySessionCode = useKeysStore((state) => state.upsertBySessionCode);
  const [key, setKey] = useState(search.key ?? "");
  const [sessionCode, setSessionCodeState] = useState(search.code ?? useSessionStore.getState().sessionCode ?? "");
  ...
  const lookup = async () => {
    ...
    try {
      ...
      setSessionCodeState(result.sessionCode);
      setFoundApplication({ applicantName: result.applicantName, status: result.status });
      upsertBySessionCode(result.sessionCode, { name: result.applicantName });
  ...
  const load = async () => {
    ...
    try {
      ...
      addSavedApplication({ accessKey: normalized, sessionCode: result.sessionCode, name: result.data?.applicant?.fullName });
