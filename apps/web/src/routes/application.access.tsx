...
function AccessPage() {
  ...
  return <main className="min-h-svh grid content-center gap-4 p-8 max-w-[760px] mx-auto"><Drawer open><DrawerContent showCloseButton={false} className="max-w-[min(34rem,calc(100vw-2rem))]"><DrawerHeader><DrawerTitle>Load an application</DrawerTitle><DrawerDescription>Use a session code to identify the child, then verify it with the private access key or QR code. A saved key can also be verified directly.</DrawerDescription></DrawerHeader><div className="flex gap-2"><Input className="min-h-12 w-full font-mono" value={sessionCode} onChange={(event) => { const v = event.target.value.toUpperCase(); setSessionCode(v); setFoundApplication(null); localStorage.setItem("aloysius-g1-application-session-code", v); }} placeholder="26ABC123 (optional)" autoComplete="off" /><Button variant="secondary" type="button" disabled={!sessionCode.trim()} onClick={() => void lookup()}>Find application</Button></div>{foundApplication && <p className="text-sm text-muted-foreground">{foundApplication.applicantName} · {foundApplication.status}. Enter the matching access key below.</p>}<Input className="min-h-12 w-full font-mono" value={key} onChange={(event) => { setKey(event.target.value); localStorage.setItem("aloysius-g1-application-key", event.target.value); }} placeholder="Private access key" autoComplete="off" autoFocus /><AccessKeyQrImporter onKey={(v) => { setKey(v); localStorage.setItem("aloysius-g1-application-key", v); }} />{error && <p className="flex items-center gap-1 text-sm text-destructive">{error}</p>}<div className="flex justify-end gap-2.5"><Button variant="secondary" type="button" onClick={() => void navigate({ to: "/" })}>Cancel</Button><Button type="button" disabled={!key.trim()} onClick={() => void load()}>Verify and load</Button></div></DrawerContent></Drawer></main>;
}
