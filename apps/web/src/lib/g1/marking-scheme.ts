 ...
 * for the CURRENT station only, measured through today - the circular
 * awards marks "only for the service period at the current service
 * station", never summed with an earlier one before a transfer: 2
 * marks/year for a period at the very school being applied to, else 1.5
 * marks/year elsewhere (another school, zonal/provincial education office, the
 * Ministry, Dept. of Examinations, Educational Publications Dept., NIE, a
 * National College of Education, or a Teacher Centre), capped at
 * `CONTRIBUTION_PATH1_YEARS_CAP` years ("only the last five years").
 * `contributionSecondPeriodEnabled` and its paired fields are a legacy
 * shape from an earlier UI that briefly summed a second, earlier station
 * in - they must never resurrect marks from a stale draft (see
 * `contributionMarks64`). The leftover months beyond the whole completed
 * years work like
 * `rateTimesYearsWithBonus`'s difficult-service convention (7.5.3): once
 * at least one whole year is complete, a remainder needs 6 months or more
 * before it earns a flat half-year bonus (nothing under that, never
 * rounded up to a full extra year). Below a first whole year, though,
 * there is no such minimum - any positive duration, even a single day,
 * already earns that same flat half mark. */
