...
import type { ScoringInputs } from "./application-store";
import {
  applyLocationChange,
...
describe("scoreCategory61 — residence & proximity", () => {
  ...
  it("awards full 20 for a title deed in the applicant's name", () => {
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant" }).breakdown[0]?.marks).toBe(20);
  });

  it("awards 16 for parents' title deed and 10 for lease and 5 for certificate", () => {
    expect(scoreCategory61({ mainDocumentType: "title-deed-parents" }).breakdown[0]?.marks).toBe(16);
    expect(scoreCategory61({ mainDocumentType: "lease-deed" }).breakdown[0]?.marks).toBe(10);
    expect(scoreCategory61({ mainDocumentType: "municipal-ds-certificate" }).breakdown[0]?.marks).toBe(5);
    expect(scoreCategory61({ mainDocumentType: "other-documents" }).breakdown[0]?.marks).toBe(4);
    expect(scoreCategory61({ mainDocumentType: "feeder-electoral-5yrs" }).breakdown[0]?.marks).toBe(15);
  });

  it("applies the deed age weighting to the document marks", () => {
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 6 }).breakdown[0]?.marks).toBe(20);
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 4.5 }).breakdown[0]?.marks).toBe(16);
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 3.2 }).breakdown[0]?.marks).toBe(12);
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 2.5 }).breakdown[0]?.marks).toBe(8);
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 1.5 }).breakdown[0]?.marks).toBe(4);
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 0.75 }).breakdown[0]?.marks).toBe(2);
    expect(scoreCategory61({ mainDocumentType: "title-deed-applicant", yearsRegistered: 0.2 }).breakdown[0]?.marks).toBe(1);
  });

  it("treats missing years as full weight", () => {
    expect(scoreCategory61({ mainDocumentType: "lease-deed" }).breakdown[0]?.marks).toBe(10);
  });
  ...
  it("awards 2.5 per registered electoral person-year capped at 25", () => {
    expect(scoreCategory61({ electoralMotherYears: 5, electoralFatherYears: 5 }).breakdown[2]?.marks).toBe(25);
    expect(scoreCategory61({ electoralMotherYears: 5, electoralFatherYears: 4 }).breakdown[2]?.marks).toBe(22.5);
    expect(scoreCategory61({ electoralMotherYears: 5, electoralFatherYears: 0 }).breakdown[2]?.marks).toBe(12.5);
    expect(scoreCategory61({ electoralMotherYears: 5, electoralFatherYears: 5.5 } as ScoringInputs).breakdown[2]?.marks).toBe(25);
  });

  it("awards five marks per selected nearby school capped at fifty", () => {
    expect(scoreCategory61({ schoolsWithinRadius: [] }).breakdown[3]?.marks).toBe(0);
    expect(scoreCategory61({ schoolsWithinRadius: schools(3) }).breakdown[3]?.marks).toBe(15);
    expect(scoreCategory61({ schoolsWithinRadius: schools(10) }).breakdown[3]?.marks).toBe(50);
    expect(scoreCategory61({ schoolsWithinRadius: schools(14) }).breakdown[3]?.marks).toBe(50);
  });
  ...
  it("caps the category total at one hundred even with maximal inputs", () => {
    const maximal = scoreCategory61({
      ...
      schoolsWithinRadius: schools(10),
    });
...
describe("scoreCategory62 — alumni", () => {
  it("awards two marks per year capped at twenty-six", () => {
    expect(scoreCategory62({ alumniYearsAtSchool: 8 }).breakdown[0]?.marks).toBe(16);
    expect(scoreCategory62({ alumniYearsAtSchool: 13 }).breakdown[0]?.marks).toBe(26);
    expect(scoreCategory62({ alumniYearsAtSchool: 20 }).breakdown[0]?.marks).toBe(26);
  });
  ...
  it("caps O/L marks at ten regardless of grade inflation", () => {
    expect(scoreCategory62({ olSubjectCount: 6, olGradeA: 6 }).breakdown[2]?.marks).toBe(10);
...
describe("scoreCategory63 — siblings", () => {
  ...
  it("awards two marks per electoral person-year capped at twenty", () => {
    expect(scoreCategory63({ electoralMotherYears: 5, electoralFatherYears: 5 }).breakdown[5]?.marks).toBe(20);
    expect(scoreCategory63({ electoralMotherYears: 3 }).breakdown[5]?.marks).toBe(6);
  });

  it("awards three marks per nearby school capped at thirty", () => {
    expect(scoreCategory63({ schoolsWithinRadius: schools(4) }).breakdown[6]?.marks).toBe(12);
    expect(scoreCategory63({ schoolsWithinRadius: schools(12) }).breakdown[6]?.marks).toBe(30);
  });
...
describe("scoreCategory64 — service & distance", () => {
  it("awards one mark per service year capped at twenty", () => {
    expect(scoreCategory64({ periodOfServiceYears: 7 }).breakdown[0]?.marks).toBe(7);
    expect(scoreCategory64({ periodOfServiceYears: 25 }).breakdown[0]?.marks).toBe(20);
  });
...
describe("scoreCategory65 — transfers", () => {
  ...
  it("awards three marks per nearby school capped at thirty", () => {
    expect(scoreCategory65({ schoolsWithinRadius: schools(10) }).breakdown[1]?.marks).toBe(30);
  });

  it("caps service period at ten", () => {
    expect(scoreCategory65({ periodOfServiceYears: 4 }).breakdown[2]?.marks).toBe(4);
    expect(scoreCategory65({ periodOfServiceYears: 40 }).breakdown[2]?.marks).toBe(10);
  });

  it("bands previous-workplace tenure", () => {
    expect(scoreCategory65({ previousWorkplacePeriodYears: 5 }).breakdown[3]?.marks).toBe(10);
    expect(scoreCategory65({ previousWorkplacePeriodYears: 2.5 }).breakdown[3]?.marks).toBe(8);
    expect(scoreCategory65({ previousWorkplacePeriodYears: 1.2 }).breakdown[3]?.marks).toBe(5);
    expect(scoreCategory65({ previousWorkplacePeriodYears: 0.5 }).breakdown[3]?.marks).toBe(0);
  });

  it("decays transfer recency from five to one", () => {
    expect(scoreCategory65({ transferElapsedYears: 0.5 }).breakdown[4]?.marks).toBe(5);
    expect(scoreCategory65({ transferElapsedYears: 2 }).breakdown[4]?.marks).toBe(4);
    expect(scoreCategory65({ transferElapsedYears: 3 }).breakdown[4]?.marks).toBe(3);
    expect(scoreCategory65({ transferElapsedYears: 4 }).breakdown[4]?.marks).toBe(2);
    expect(scoreCategory65({ transferElapsedYears: 5 }).breakdown[4]?.marks).toBe(1);
    expect(scoreCategory65({ transferElapsedYears: 6 }).breakdown[4]?.marks).toBe(0);
  });
  ...
  it("reaches exactly one hundred at maxima", () => {
    const maximal = scoreCategory65({
      ...
      schoolsWithinRadius: schools(10),
      periodOfServiceYears: 10,
      previousWorkplacePeriodYears: 5,
      transferElapsedYears: 1,
...
describe("scoreCategory66 — foreign employment", () => {
  it("bands continuous-abroad periods", () => {
    expect(scoreCategory66({ periodAbroadYears: 5 }).breakdown[0]?.marks).toBe(25);
    expect(scoreCategory66({ periodAbroadYears: 2 }).breakdown[0]?.marks).toBe(15);
    expect(scoreCategory66({ periodAbroadYears: 1 }).breakdown[0]?.marks).toBe(10);
    expect(scoreCategory66({ periodAbroadYears: 0.5 }).breakdown[0]?.marks).toBe(0);
  });
  ...
  it("awards three and a half marks per nearby school capped at thirty-five", () => {
    expect(scoreCategory66({ schoolsWithinRadius: schools(4) }).breakdown[2]?.marks).toBe(14);
    expect(scoreCategory66({ schoolsWithinRadius: schools(10) }).breakdown[2]?.marks).toBe(35);
    expect(scoreCategory66({ schoolsWithinRadius: schools(20) }).breakdown[2]?.marks).toBe(35);
  });
...
describe("scoreCategory dispatcher", () => {
  ...
  it("keeps totals non-negative for nonsensical negative inputs", () => {
    expect(scoreCategory64({ periodOfServiceYears: -5 }).total).toBe(0);
