 ...
 * which reads DATABASE_URL from process.env at that instant. A static import
...
describe("admin procedure guard", () => {
  it("rejects an unauthenticated caller with UNAUTHORIZED", async () => {
    await expect(
      call(context.appRouter.admin.overview, { intakeYear: "2027" }, { context: anonymousContext() }),
    ).rejects.toMatchObject({ code: "UNAUTHORIZED" });
  });

  it("rejects a signed-in non-admin caller with FORBIDDEN", async () => {
    await expect(
      call(context.appRouter.admin.overview, { intakeYear: "2027" }, { context: userContext("user") }),
...
describe("application create → get → update → submit", () => {
  ...
  it("round-trips a full application lifecycle", async () => {
    ...
    await call(context.appRouter.application.submit, { accessKey: created.accessKey }, { context: anonymousContext() });
...
describe("intake year isolation", () => {
  ...
  it("admin.overview only counts applications for the requested intake year", async () => {
    const overviewA = await call(context.appRouter.admin.overview, { intakeYear: yearA }, { context: userContext("admin") });
    const overviewB = await call(context.appRouter.admin.overview, { intakeYear: yearB }, { context: userContext("admin") });
...
describe("application marks: applicant-submitted vs admin-authoritative isolation", () => {
  ...
  beforeAll(async () => {
    ...
    const overview = await call(context.appRouter.admin.overview, { intakeYear }, { context: userContext("admin") });
  ...
  it("a forged applicant-submitted mark never appears in the admin's authoritative view", async () => {
    await call(
      ...
      { accessKey, marks: [{ categoryType: "6.1", total: 100, breakdown: [{ label: "Forged", marks: 100, max: 100 }] }] },
      { context: anonymousContext() },
    );
  ...
  it("an admin-saved mark never appears in the applicant's own indicative-marks view", async () => {
    await call(
      ...
      { applicationId, categoryType: "6.1", breakdown: [{ label: "Verified", marks: 42, max: 100 }], total: 42 },
      { context: userContext("admin") },
    );

    const applicantMarks = await call(context.appRouter.application.getMarks, { accessKey }, { context: anonymousContext() });
  ...
  it("rejects an out-of-range indicative mark", async () => {
    await expect(
      call(
        context.appRouter.application.saveIndicativeMarks,
        { accessKey, marks: [{ categoryType: "6.1", total: 999, breakdown: [{ label: "Too high", marks: 999, max: 100 }] }] },
        { context: anonymousContext() },
      ),
