 ...
 * `@aloysius-admissions/env/server` requires at safe, isolated test values for the
 ...
 * so importing them first would bind to whatever `TURSO_DATABASE_URL` was already
 * set (or none). Use a dynamic `await import(...)` after calling this.
 *
 * `TURSO_DATABASE_URL` is always force-overridden (never reused from a real `.env`)
 * so a test can never accidentally run against a real dev/prod database.
 */
export function provisionTestDatabase(): TestDatabase {
  const dir = mkdtempSync(join(tmpdir(), "aloysius-admissions-test-db-"));
  ...
  const result = spawnSync(
    ...
    {
      cwd: dbPackageDir,
      env: { ...process.env, TURSO_DATABASE_URL: `file:${databasePath}` },
      encoding: "utf-8",
      timeout: 60_000,
  ...
  return {
    ...
    cleanup: () => {
      ...
      // query (the libSQL client doesn't always release the handle the
